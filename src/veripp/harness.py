...
REACHABILITY_MESSAGE = "veripp: harness is reachable under its assumptions"

#: Defined for the probe run only; turns VERIPP_REACHED() into an assertion
#: that always fails (see veripp/contracts.hpp).
REACHABILITY_PROBE_MACRO = "VERIPP_REACHABILITY_PROBE"

_REACHED_RE = re.compile(r"^[ \t]*VERIPP_REACHED[ \t]*\([ \t]*\)[ \t]*;", re.M)


def has_reachability_marker(code: str) -> bool:
    """Whether `code` carries the probe point every generated harness ends with."""
    return _REACHED_RE.search(scrub(code)) is not None


def reachability_variant(code: str) -> str | None:
    """`code` with an always-failing assertion at the end of main, or None.

    A precondition that contradicts itself, or merely cannot hold together
    with the harness's own bounds, makes the call unreachable -- and an
    unreachable program satisfies every property. ESBMC reports VERIFICATION
    SUCCESSFUL and the "proof" means nothing. Running this variant inverts
    the question: a reachable main must fail the assertion, so if the variant
    verifies, the original proof was vacuous.

    Generated harnesses need none of this: they end with VERIPP_REACHED(),
    and the probe is the same file compiled with REACHABILITY_PROBE_MACRO.
    This is for a file checked through its own main. The assertion goes
    before main's final `return`, or before its closing brace when it has
    none. With no main to put it in, the answer is None: a result that cannot
    be probed is not confirmed, rather than confirmed by a probe that checks
    nothing.
    """
    scrubbed = scrub(code)
    probe = f'__ESBMC_assert(0, "{REACHABILITY_MESSAGE}");'
    for match in re.finditer(r"\bmain\s*\(", scrubbed):
        try:
            rparen = match_bracket(scrubbed, scrubbed.index("(", match.end() - 1))
        except SignatureError:
            continue
        brace = scrubbed.find("{", rparen)
        if brace == -1 or scrubbed[rparen + 1 : brace].strip():
            continue  # a declaration or a call, not the definition
        try:
            end = match_bracket(scrubbed, brace)
        except SignatureError:
            continue
        # A final `return` on a line of its own ends every path that falls
        # through; the probe goes in front of it. Anywhere else -- the tail
        # of an unbraced `if` -- it would change what the code does.
        final = re.search(
            r"^[ \t]*return\b[^;{}]*;\s*\Z", scrubbed[brace + 1 : end], re.M
        )
        at = brace + 1 + final.start() if final else end
        return f"{code[:at]}    {probe}\n{code[at:]}"
    return None
